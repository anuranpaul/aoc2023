...
const solution = solve(readLines(`./example.txt`))
...
function solvePart1(inputLines) {
...
function parseLineForPart1(line) {
  return line
    .split(":")[1]
    .trim()
    .split(/\s+/)
...
function solvePart2(inputLines) {
  const [timeLine, distanceLine] = inputLines
...
function parseLineForPart2(line) {
  const num = line
    .split(":")[1]
    .trim()
    .split(/\s+/)
