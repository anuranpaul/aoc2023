...
const input = fs.readFileSync(path.resolve("input.txt"), "utf8").split("\n").map(x => x.split(" ").map(y => parseInt(y)));
let result = 0;
...
for (const line of input) {
    ...
    for (let i = 1; i < differences.length; i++) {
        ...
        currentDifference.push(currentDifference[currentDifference.length - 1] + previousDifference[previousDifference.length - 1]);
    }
    const lastDifference = differences[differences.length - 1];
    result += lastDifference[lastDifference.length - 1];
}
